...
bool CTFBotMainAction::IsImmediateThreat( const CBaseCombatCharacter *subject, const CKnownEntity *threat ) const
{
	...
	if ( threatPlayer == NULL )
	{
		...
		if ( sentry && !sentry->HasSapper() && !sentry->IsPlacing() )
		{
																		 ...
																		 const CKnownEntity *threat2 ) const
{
	...
	*/
	if ( shouldFearSentryGuns )
	{
		...
		if ( sentry1 && me->IsRangeLessThan( sentry1, SENTRY_MAX_RANGE ) && !sentry1->HasSapper() && !sentry1->IsPlacing() )
		{
			// in range of a visible sentry!
			if ( sentry2 && me->IsRangeLessThan( sentry2, SENTRY_MAX_RANGE ) && !sentry2->HasSapper() && !sentry2->IsPlacing() )
			{
