...
void CTFBotVision::UpdatePotentiallyVisibleNPCVector( void )
{
	if ( m_potentiallyVisibleUpdateTimer.IsElapsed() )
	{
		...
		for ( int i=0; i<IBaseObjectAutoList::AutoList().Count(); ++i )
		{
			...
			// conn: rn we have 2 teleporter classes, valve later collapsed these 2 into a single class, we should copy that
			else if (pObj->ObjectType() == OBJ_TELEPORTER_ENTRANCE )
			{
				m_potentiallyVisibleNPCVector.AddToTail( pObj );
			}
			else if (pObj->ObjectType() == OBJ_TELEPORTER_EXIT)
			{
				m_potentiallyVisibleNPCVector.AddToTail( pObj );
			}
		}
